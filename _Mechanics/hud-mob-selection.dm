...
TitleScreen
	...
	New(mob/m)
		...
		if(background.Width() <= Constants.ICON_WIDTH)
			add(0, 0, icon = background, width = m.client.VIEW_WIDTH, height = m.client.VIEW_HEIGHT)

		// otherwise center it in the screen
		else
			var/ix = Constants.ICON_WIDTH * m.client.VIEW_WIDTH * 0.5 - background.Width() * 0.5
			var/iy = Constants.ICON_HEIGHT * m.client.VIEW_HEIGHT * 0.5 - background.Height() * 0.5

			add(ix, iy, icon = background)

		add(0, 16, "", maptext_width = m.client.VIEW_WIDTH * Constants.ICON_WIDTH, maptext = "<text align=center>press SPACE BAR to begin</text>", layer = layer + 1)
