...
InfoBar
	...
	New(mob/m)
		...
		background = add(0, 0, "info-bar-[bottom ? "bottom" : "top"]", width = owner.client.VIEW_WIDTH)
		caption = add(4, 16, "", maptext_width = owner.client.VIEW_WIDTH * Constants.ICON_WIDTH - 8, maptext_height = 16, layer = layer + 1)
		counter = add(4, 16, "", maptext_width = owner.client.VIEW_WIDTH * Constants.ICON_WIDTH - 8, maptext_height = 16, layer = layer + 1)

		if(bottom)
			pos(0, 0)
		else
			pos(0, (owner.client.VIEW_HEIGHT - 1) * Constants.ICON_HEIGHT)
	...
	hide()
		...
		if(owner)
			...
			if(bottom)
				...
				if(owner.info_box)
					owner.info_box.pos(owner.client.VIEW_WIDTH * Constants.ICON_WIDTH - 128 - 8, 8)

			// top of the screen
			else
				if(owner.health_meter)
					owner.health_meter.pos(8, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 24)

				if(owner.mana_meter)
					owner.mana_meter.pos(8, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 40)

				if(owner.conditions_bar)
					owner.conditions_bar.pos(0, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 62)
	...
	show()
		...
		if(owner)
			...
			if(bottom)
				...
				if(owner.info_box)
					owner.info_box.pos(owner.client.VIEW_WIDTH * Constants.ICON_WIDTH - 128 - 8, 24)

			// top of the screen
			else
				if(owner.health_meter)
					owner.health_meter.pos(8, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 40)

				if(owner.mana_meter)
					owner.mana_meter.pos(8, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 56)

				if(owner.conditions_bar)
					owner.conditions_bar.pos(0, owner.client.VIEW_HEIGHT * Constants.ICON_HEIGHT - 78)
